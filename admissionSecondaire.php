<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">
    <link rel="stylesheet" href="laReussite.css">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-OERcA2EqjJCMA+/3y+gxIOqMEjwtxJY7qPCqsdltbNJuaOe923+mo//f6V8Qbsw3" crossorigin="anonymous"></script>
    <script src="https://kit.fontawesome.com/431fa92df2.js" crossorigin="anonymous"></script>
    <title>Admission secondaire</title>
</head>
<body>
<?php
    include('header.php');
    ?>
    <main>
    <div class="admission" >
            L’école de la reussite se situe au centre de Dakar dans le quartier Kermel au plateau, dans un batiment neuf et moderne.

            Elle accueille les élèves des cycles élémentaire (CP au CM2) et maternelle (TPS à GS).

            A l’enseignement d’excellence que propose l’école Aimé Césaire s’ajoute une dimension internationale, à travers son programme bilingue dispensé selon les méthodes, programmes et instructions pédagogiques en usage en France.</div>

    </main>
    <?php
    include('footer.php');
    ?>
    <script src="laReussite.js"></script>
</body>
</html>